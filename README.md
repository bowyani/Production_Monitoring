...
| UC-5  | นำเข้าข้อมูล Job ด้วยมือ (Manual Fallback)                          | Chief Operator                    | อัปโหลด CSV บันทึกผล Job สำหรับเครื่องที่เชื่อมต่อไม่ได้ (`dataSource=MANUAL_CSV`)                                                                                                      |
...
| UC-12 | ลงทะเบียนเครื่องจักรใหม่เข้าระบบ                                    | Chief Operator                    | เลือก ERP asset ที่มีอยู่แล้วมาผูก — โหมด **Simulator** (`dataSource=SIMULATOR`) สั่ง start container ผ่าน Docker socket; โหมด **Physical Machine** กรอก Gateway + Modbus address บันทึกเป็น config รอ Gateway มา poll ไม่ start container |
...
PostgreSQL, 11 ตาราง (normalize ไม่ denormalize — join ได้ถูกต้อง ขยาย field ง่ายกว่าระยะยาว):

| ตาราง                   | ใช้เก็บ                                                                                                                                                                                          | Field สำคัญ                                                                                                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `machines`              | สถานะการเชื่อมต่อของเครื่องจักรที่ลงทะเบียน (operational registry — ไม่มี spec/cost แล้ว ดู `erp_machine_assets`)                                                                                | `machine_id` (PK, = FK ไปยัง `erp_machine_assets.asset_id`), `status`, `data_source` (`SIMULATOR`/`MODBUS_GATEWAY`/`MANUAL_CSV`), `last_seen_at`, `is_active`, `last_maintenance_at`                                |
| `gateways`              | Protocol Gateway / Edge Adapter — กล่องที่ poll PLC จริงผ่าน Modbus แล้ว republish เป็น MQTT (Prototype ใช้ mock ตัวเดียว)                                                                        | `gateway_id` (PK), `ip_address`, `location`, `last_heartbeat_at`, `status` (`ONLINE`/`OFFLINE`)                                                                                                                    |
| `machine_connection_configs` | รายละเอียดการเชื่อมต่อของแต่ละเครื่อง (1:1 กับ `machines`) — SIMULATOR/MANUAL_CSV มีแค่ `connection_type`, ส่วน MODBUS_* มี gateway + Modbus addressing ด้วย                                | `machine_id` (PK+FK), `connection_type` (`SIMULATOR`/`MODBUS_TCP`/`MODBUS_RTU`/`MANUAL_CSV`), `gateway_id` (FK nullable), `modbus_slave_id`, `modbus_ip`, `modbus_port`, `register_map` (JSON); unique `(gateway_id, modbus_slave_id)` |
...
| `GET /admin/machines`, `POST /admin/machines`, `PATCH /admin/machines/:id`                           | ลงทะเบียน/เปิด-ปิดเครื่องจักร (path **Simulator**) — `POST` รับ `{ assetId, dataSource }` (`SIMULATOR`/`MANUAL_CSV`, เลือกจาก `erp_machine_assets` ที่มีอยู่) เขียน `machine_connection_configs` ให้อัตโนมัติ และสั่ง start simulator container ให้ถ้า `dataSource=SIMULATOR`                                                                                    |
| `POST /admin/machines/manual-register`                                                               | ลงทะเบียนเครื่องจักร**จริง** (path **Physical Machine**) — รับ `{ assetId, connectionType (MODBUS_TCP/RTU), gatewayId, modbusSlaveId, modbusIp, modbusPort, registerMap }` validate ด้วย Zod, ไม่ start container, กัน `modbus_slave_id` ชนกันภายใน gateway เดียวกัน (400)                                                                                          |
| `GET/POST /admin/gateways`, `PATCH/DELETE /admin/gateways/:id`, `GET /admin/gateways/:id/machines`   | CRUD ของ Gateway + list เครื่องที่ผูกกับ gateway นั้น (ลบ gateway ไม่ได้ถ้ายังมีเครื่องผูกอยู่ — 409)                                                                                                                                            |
| `POST /admin/gateways/:id/bootstrap`, `POST /admin/gateways/:id/heartbeat`                           | Endpoint ภายในที่ mock-gateway เรียกเองตอน start (สร้าง Gateway row แบบ idempotent) และ ping liveness ทุกไม่กี่วินาที (อัปเดต `last_heartbeat_at`, flip `status`→ONLINE); watchdog เป็นตัว flip กลับเป็น OFFLINE เมื่อ heartbeat ขาด                |
...
| `POST /admin/import/jobs`                                                                            | Import Job/Production data จาก CSV สำหรับเครื่องที่เชื่อมต่อไม่ได้ (`dataSource=MANUAL_CSV`)                                                                                                                                                       |
...
> **Prototype นี้ทำ mock ของ Gateway ตัวนี้ไว้แล้ว** — `mock-gateway/` (Node.js) รันเป็น service ใน `docker compose up`: register ตัวเองเป็น `gateways` row, self-seed เครื่อง Modbus 2 เครื่อง (`IMM-51`, `IMM-52`) ผูกกับตัวเอง, แล้ว "poll" (สังเคราะห์ค่า ไม่มี RS-485 จริง) → publish `factory/<id>/telemetry|job|alarm` ด้วย schema เดิม + heartbeat ให้หน้า Gateway Management เห็นสถานะ ONLINE. Backend/Broker/Dashboard ไม่ต้องแก้อะไรเพิ่มเพื่อรับข้อมูลจากมัน — พิสูจน์ข้อความข้างบนนี้ด้วยของจริง
...
คำสั่งเดียวจะสร้างและรันครบทั้งระบบ: Mosquitto, PostgreSQL (migrate schema อัตโนมัติ), Backend, Dashboard, Machine Simulator 3 เครื่อง (`IMM-01`, `IMM-02`, `IMM-03`) ที่ลงทะเบียนตัวเองและเริ่ม publish ทันที และ Mock Protocol Gateway 1 ตัว (`GW-01`) ที่ลงทะเบียนเครื่อง Modbus `IMM-51`/`IMM-52` ผูกกับตัวเองแล้ว poll + republish เป็น MQTT — ไม่ต้องตั้งค่าอะไรเพิ่ม

- Dashboard: http://localhost:5173 (Operation / Production / Performance / Executive KPI / Machine Management / Gateway Management / Manual Import / Simulator Tuning / Admin / ERP) — System Health and Audit Log both live inside Admin now, not separate pages
...
```
simulator/    Machine Simulator (Node.js + mqtt.js) — จำลองเครื่องฉีดพลาสติกหลาย instance (publish MQTT เอง)
mock-gateway/ Mock Protocol Gateway (Node.js) — จำลอง PLC→Modbus→Gateway→MQTT: poll เครื่อง Modbus แล้ว republish เป็น MQTT topic เดิม
backend/      Node.js/TypeScript — MQTT subscriber + REST API v1 + WebSocket + Prisma/PostgreSQL
dashboard/    React + Vite — Operation / Production / Performance / Executive KPI / ERP / Machine Management / Gateway Management / Admin / Simulator Tuning views
...
erd_diagram.svg               ERD ของ Database Structure (11 ตาราง)
